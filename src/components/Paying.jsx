...
const Paying = ({ eventData, selectedTickets }) => {
  ...
  // VOUCHER
  const [showVoucherModal, setShowVoucherModal] = useState(false); // Modal danh sách
  ...
  const [loadingVouchers, setLoadingVouchers] = useState(false); // Loading khi fetch voucher
  const [voucherCode, setVoucherCode] = useState(""); // Nhập voucher
  const [voucherError, setVoucherError] = useState(""); // Lỗi voucher
  ...
  const handleApplyVoucherCode = () => {
    ...
    const match = availableVouchers.find(v => {
      ...
      const validDate = now >= startDate && now <= endDate;
      const matchCode = v.code.trim().toUpperCase() === voucherCode.trim().toUpperCase();
      const matchEvent = v.event_id === eventData.id;

      return matchCode && validDate && matchEvent;
    });
    ...
    <main className="flex py-9 max-w-7xl mx-auto px-4 relative">
      <div className="grid grid-cols-12 w-full relative">
        <section className="relative col-span-8 translate-y-[-1rem] animate-fade-in opacity-0 [--animation-delay:200ms] mr-4 bg-secondary rounded-[10px]">
          <div className="p-8">
            <h2 className="font-extrabold text-white text-xl text-center mb-8">THANH TOÁN</h2>


            <div className="space-y-4">
              {/* Thông tin nhận vé */}
              <div className="bg-white rounded-[5px] p-6">
                <h3 className="font-bold text-primary text-lg mb-2">Thông tin nhận vé</h3>
              ...
              {/* --- MÃ KHUYẾN MÃI (UI MỚI) --- */}
              <div className="bg-white rounded-[5px] p-6">
                <h3 className="font-bold text-primary text-lg mb-4">Mã khuyến mãi</h3>
                ...
                {!appliedVoucher ? (
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <span className="font-bold text-secondary text-sm">Thêm voucher</span>
                    </button>
                  </div>
                ) : (
                  // Thẻ Voucher đã chọn
                  <div className="flex items-center justify-between bg-green-50 border border-green-200 p-4 rounded-lg shadow-sm">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-green-700 text-lg">{appliedVoucher.code}</p>
                    ...
                    <button onClick={handleRemoveVoucher} className="text-gray-400 hover:text-red-500 p-2 hover:bg-red-50 rounded-full transition-colors">
                      <X size={20} />
                    </button>
                  </div>
                )}
              ...
              <div className="bg-white rounded-[5px] p-6">
                <h3 className="font-bold text-primary text-lg mb-4">Phương thức thanh toán</h3>
                <div className="space-y-4">
                  {paymentMethods.map((method) => (
                    <label key={method.id} htmlFor={method.id} className="flex items-center gap-3 cursor-pointer select-none p-2 hover:bg-gray-50 rounded-lg transition-colors">
                      <input
                        type="radio" name="paymentMethod" id={method.id} value={method.id}
        ...
        <aside className="relative col-span-4 animate-fade-in opacity-0 [--animation-delay:400ms] ">
          <div className="bg-white rounded-lg  p-6 sticky top-24">
            <h2 className="font-bold text-black text-xl mb-4">Thông tin thanh toán</h2>
            <div className="space-y-4 mb-4 max-h-[300px] overflow-y-auto pr-2">
              {selectedTickets.map((ticket, idx) => (
                <div key={idx} className="flex items-center justify-between text-sm">
                  <div className="flex-1 mr-2">
                    <p className="font-bold text-secondary">{ticket.name}</p>
                    <p className="font-medium italic text-gray-500">{formatPrice(ticket.price)}</p>
                  </div>
                  <span className="font-bold text-primary text-base whitespace-nowrap">x {ticket.quantity}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center justify-between pt-4 border-t-2 border-dashed border-gray-300">
                ...
                <span className="font-extrabold text-primary text-2xl">{formatPrice(finalTotal)}</span>
              </div>
            </div>
